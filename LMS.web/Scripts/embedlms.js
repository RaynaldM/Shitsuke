///#source 1 1 /TypeScripts/Assets/EmbedLms.js
/// <reference path="../../scripts/typings/base.d.ts" />
var EmbedLms = (function () {
    function EmbedLms() {
    }
    EmbedLms.Ping = function (appCode, bearerToken, userId, page, useFeedBack, lang) {
        var _this = this;
        if (page === void 0) { page = location.pathname; }
        if (useFeedBack === void 0) { useFeedBack = true; }
        if (lang === void 0) { lang = navigator.language || navigator.systemLanguage || navigator.userLanguage || "en"; }
        // first if there is no JQuery, inject it
        if (window.jQuery) {
            this.go(appCode, bearerToken, userId, page, useFeedBack, lang);
        }
        else
            this.injectJS("https://ajax.googleapis.com/ajax/libs/jquery/2.2.2/jquery.min.js", false, function () {
                _this.go(appCode, bearerToken, userId, page, useFeedBack, lang);
            });
    };
    EmbedLms.go = function (appCode, bearerToken, userId, page, useFeedBack, lang) {
        ...
        if (useFeedBack) {
            ...
            lmsloadHtml("/html/feedbackroot.min.html", "body")
                .then(function () {
                // set click event
                $("#lmsFeedBack-btn").on("click", function () {
                    if (alreadyLoaded) {
                        ...
                        // When cliked
                        // load JS files
                        $.getScript(window.lms + "/scripts/feedback.min.js", function () {
                            feedback.Ready(bearerToken, appCode, userId, page, lang);
                            alreadyLoaded = true;
                        });
                    }
    ...
    EmbedLms.injectJS = function (url, fromRoot, callback) {
        if (fromRoot === void 0) { fromRoot = true; }
        // Inject JS process
        var dom = document;
        var scriptTag = "script";
        // Set script element 
        var embedscript = dom.createElement(scriptTag);
...
function lmsloadHtml(url, element, callback) {
...
//	}
//# sourceMappingURL=EmbedLms.js.map
