...
module Base {
    export class Helpers {
        ...
        private static setTimeAgo(element: JQuery, delay: number) {
            var ctimout = parseInt(element.attr("ctimeout"));
            if (ctimout >= 0)
                clearTimeout(ctimout);
            var value = element.attr("data-timeago");
            if (value) {
        ...
        static SetAjaxWaiter(jqSelector: string = "#infinitLoad"): void {
            ...
            $(document).on("ajaxStart", ( /*e, xhr, settings, exception*/) => {
                this.waitPanel.show();
            }).on("ajaxStop", ( /*e, xhr, settings, exception*/) => {
                this.waitPanel.hide();
            });
        }

        static SetAjaxWaiterToastR(): void {
            $(document)
                .on("ajaxStart", ( /*e, xhr, settings, exception*/) => {
                    toastr.info("Loading");
                })
                .on("ajaxStop", ( /*e, xhr, settings, exception*/) => {
        ...
        static CancelBubble(e: JQueryEventObject): void {
            var evt = e ? e : window.event;
            if (evt.stopPropagation) evt.stopPropagation();
            if (evt.cancelBubble != null) evt.cancelBubble = true;
        }

        static RadioButtonClick(selector: string, gridName: string,
			funcToSetVar: Function, callAction: Function): JQueryPromise<any> {
			var serverVarName = gridName + "|" + selector;
			...
			element.unbind("click")
				.on("click", (e: JQueryEventObject) => {
					...
					obj.addClass("active");
					var choice = parseInt(obj.attr("data-choice"));
					this.JsonAjaxService("POST", "/Home/SetUserParams",
						{
							id: serverVarName,
							value: choice
						}).fail(() => { toastr.warning("Can't save this parameters") });
					callAction && callAction(choice);
				});

			// Get the last value setted by user
			return this.JsonAjaxService("GET", "/Home/GetUserParams/" + serverVarName)
				.done((result) => {
					if (result) {
						funcToSetVar && funcToSetVar(result);
						element.removeClass("active");
						$(selector + "[data-choice='{0}']".format(result))
							.addClass("active");
					}
				});
